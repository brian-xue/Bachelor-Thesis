...
static struct rte_mbuf *create_packet(void)
{
    struct rte_mbuf *m;
    ...
    math_data_len = sizeof(math_header_t);
    ...
    if (pkt_data_len > sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr) + 
        sizeof(struct rte_udp_hdr) + sizeof(math_header_t)) {
        
        payload = (char *)(math_hdr + 1);
        size_t padding_size = pkt_data_len - (sizeof(struct rte_ether_hdr) + 
                             sizeof(struct rte_ipv4_hdr) + sizeof(struct rte_udp_hdr) + 
                             sizeof(math_header_t));
